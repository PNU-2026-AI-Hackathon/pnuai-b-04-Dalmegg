...
닮은살걀은 도시의 유휴공간을 소규모 화훼 생산 공간으로 전환하는 MiniFarm 서비스입니다. 운영자는 관리자 웹에서 재배 환경, 꽃 상품·재고, 체험 예약을 관리하고, 소비자는 모바일 앱에서 꽃을 주문하고 계란껍질 수거 및 꽃꾸 체험에 참여합니다. Backend는 두 서비스와 MiniFarm 센서 데이터를 연결합니다.
...
| 관리자 웹 | 관리자 인증, 운영 Dashboard, 센서 최신값·이력 조회, 꽃 상품·재고 관리, 체험 예약 관리 |
| Backend | REST/WebSocket API, JWT 권한 관리, 주문·예약·수거·리워드 도메인, MQTT 센서 데이터 처리 |
...
3. **생산 이후의 판로 연결**: 화훼 상품·재고 관리와 소비자 주문·체험 예약을 서비스 안에서 지원합니다.
4. **자원순환 참여**: 계란껍질 수거 신청·기여도·리워드 기능으로 소비자의 참여 흐름을 설계합니다.
...
현재 센서 연동의 구현 범위는 **Sensor → MQTT → Backend → Database → 관리자 웹 조회**입니다. 관리자 웹에서 펌프·릴레이 등 하드웨어를 제어하는 경로는 아직 구현하지 않았습니다.
...
Final MVP의 운영 화면은 현재값과 설정 범위를 비교하는 **규칙 기반 운영**을 사용합니다. 아래 수치는 발표 시연에서 사용한 꽃 재배 환경의 설정 기준입니다.

| 환경 항목 | 설정 범위 | 시연 예시 | 처리 |
| --- | ---: | ---: | --- |
| 온도 | 20–26°C | 24.2°C | 정상 |
| 습도 | 60–75% | 68% | 정상 |
| 조도 | 15,000–25,000 lx | 17,200 lx | 정상 |
| 토양 수분 | 50–70% | 42% | 이상 표시 및 우선 조치 안내 |

토양 수분처럼 범위를 벗어난 항목은 Dashboard에서 우선 표시하고, 운영자에게 센서 상세 확인과 급수 상태·관수 장치 점검 같은 조치 안내를 제공합니다. 현재 규칙은 생육 예측 모델이 아니라 설정 범위 비교에 기반합니다.
...
| 관리자 웹 Demo Mode | Mock 데이터임을 전제로 화면·운영 흐름 확인 |
| 소비자 앱이 서버에 연결되지 않음 | 화면 확인용 Demo 데이터를 표시하며 주문·수거·예약은 서버로 전송하지 않음 |
...
아래 상태는 저장소 코드와 검증 결과를 기준으로 정리했습니다.

| 영역 | 기능 | 상태 |
| --- | --- | --- |
| App | 회원가입·로그인·토큰 갱신 | 구현 |
| App | 꽃 전체 목록 조회·단일 꽃 주문 | 구현 |
| App | 계란껍질 수거 신청·내 수거 내역 | 구현 |
| App | 꽃꾸 체험 목록·예약, 마이페이지 기여도·리워드·예약 표시 | 구현 |
| App | 꽃집 목록·꽃집별 상품 흐름 | 미구현 |
| App | 채팅·맞춤 부케·리뷰·즐겨찾기 API 연동 | 미연동 |
| Web | 관리자 회원가입·로그인·꽃집 생성 | 구현 |
| Web | MiniFarm Dashboard, 센서 최신값·이력 Chart | 구현 (실제 API 우선, Mock Fallback) |
| Web | 꽃·이미지·재고 CRUD | 구현 |
| Web | 체험 예약 조회·상태 변경 | 구현 |
| Web | 주문·채팅·수거 관리 화면 | 미구현 |
| Backend | 인증·권한, 상품·주문·재고, 체험·예약, 수거·리워드 | 구현 |
| Backend | 맞춤 부케·채팅 REST/WebSocket, 리뷰·즐겨찾기 API | 구현 |
| Backend | MQTT 센서 수집, 최신값·이력 저장·조회 | 구현 |
| IoT 제어 | 관리자 웹 기반 급수 제어, Pump/Relay 제어 Message 발행 | 미구현 |
| Hardware | 실물 MiniFarm Firmware·Pin 연결 | 저장소 기준 검증 필요 |
...
- 서버에 연결할 수 없을 때는 화면 확인을 위한 Demo 데이터를 표시합니다.
- Demo 데이터에서는 주문·수거 신청·체험 예약이 서버에 전송되지 않습니다.
- 인증 토큰은 현재 앱 실행 중에만 유지됩니다. 운영 배포 전에는 Keychain/Keystore 기반 보안 저장소 연동이 필요합니다.
...
- API 연결이 실패하거나 Demo Mode일 때 일부 운영 데이터 Mock 표시
...
조치 완료 상태와 내용은 현재 Web Browser Local Storage에 저장됩니다. 운영 이력의 Backend 영구 저장과 하드웨어 제어 명령 연동은 향후 고도화 범위입니다.

### 3.4 Backend 및 IoT 연동
...
- 맞춤 부케와 사용자·관리자 1:1 채팅 REST/WebSocket API를 제공합니다. 단, 소비자 앱과 관리자 웹의 관련 화면 연동은 향후 과제입니다.
...
기본 센서 이력 저장 간격은 60초입니다. 현재는 센서 조회까지 구현되어 있으며, Backend가 하드웨어로 제어 Message를 발행하는 기능은 구현하지 않았습니다.
...
| 소비자 앱 | `flutter analyze`, `flutter test`, Emulator E2E | Flutter SDK·Emulator 환경에서 추가 검증 필요 |

`pytest`의 Skip 1건은 실제 MySQL 연결이 불가능한 경우 건너뛰도록 작성된 Database 연결 테스트입니다. Web은 별도 Unit/Component Test Framework가 아직 없으므로, 정적 검사·Production Build와 주요 화면의 수동 동선 점검으로 검증했습니다.

### 3.7 실행 로드맵

| 기간 | 목표 | 검증 기준 |
| --- | --- | --- |
| 단기 | 관리자 웹 → Backend → MQTT → Pump/Relay의 급수 제어 경로 연결 | 이상 감지부터 급수 요청까지의 전체 경로 |
| 중기 | 유휴상가 한 곳에서 실제 MiniFarm 운영 | 센서·조치·판매 운영 데이터 축적 |
| 장기 | 축적 데이터 기반 생육 최적화 모델 검증 | 환경값·조치·결과가 결합된 학습 데이터셋 활용 |

이와 함께 소비자 앱의 꽃집별 상품 탐색, 채팅·맞춤 부케·리뷰·즐겨찾기 연동, Flutter 실기기 End-to-End 검증, MySQL을 포함한 배포 구성과 CI 구축을 진행합니다.

### 3.8 프로토타입 시연 범위

발표 시연은 아래의 다섯 단계를 기준으로 구성합니다.
...
저장소 코드에서 검증된 범위는 MQTT 수신, Database 저장, 관리자 웹 조회입니다. 실물 센서·펌프의 동작 여부는 시연 환경에서 별도로 확인하며, Web 기반 제어 API는 현재 미구현입니다.
