...
> • **친환경 자원순환:** 지역 자원을 재배 환경과 연결해 폐기물을 줄이고 지속 가능한 운영을 실현
...
- **프로그래밍 언어**
    ...
    - Backend : Python
- **프레임워크**
    - Frontend : Web : React + Tailwind CSS / App: Flutter
    - Backend : FastAPI
- **기타**
    - 웹서버 : Nginx
    - 컨테이너화 : Docker
...
- 앱을 통해 지역 자원 순환 활동 이력을 기록하고 누적 기여량을 시각화하여 확인한다.
...
| 기능 | 사용자 | 설명 | 상태 |
| --- | --- | --- | --- |
| 자원 순환 활동 등록/조회 | 일반 소비자 | 지역 참여 기록 및 누적 데이터 확인 | 개발 예정 |
...
운영자는 데이터 기반 재배 환경 관리와 지역 기반 거래 플랫폼을 통해 화훼 생산과 판매를 효율적으로 운영할 수 있도록 설계하였다.
소비자는 지역에서 재배된 신선한 꽃을 가까이에서 만나고, 체험과 리워드를 통해 지역 순환 활동에 참여한다. 맞춤 꽃다발 제작과 체험 예약까지 연결해 단순한 주문 플랫폼을 넘어 지역 화훼 경험을 제공한다.
...
1. **지속 가능한 재배 운영** : 지역 자원 순환 활동을 연계해 재배 비용을 절감하고 친환경 생산 기반을 강화
...
또한 지역 자원 순환 활동을 재배 운영에 연계해 폐기물 발생을 줄이고 지속 가능한 농업 구조를 실현한다. 이를 통해 단순 생산 중심의 스마트팜을 넘어, 환경 데이터와 지역 참여를 연결하는 지속 가능한 화훼 운영 모델을 구축하고자 한다.
