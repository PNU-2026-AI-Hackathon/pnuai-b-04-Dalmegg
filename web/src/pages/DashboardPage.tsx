    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="mt-5 grid gap-px overflow-hidden border border-[#d9e0d7] bg-[#d9e0d7] md:grid-cols-[1.15fr_1fr]">
        ...
        <div className="bg-[#fff5f8] p-6 md:p-7">
          ...
          <button type="button" className="mt-6 inline-flex items-center gap-2 bg-rose-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-rose-800">센서 상세 확인 <ArrowRight size={15} /></button>
        </div>
      </section>
      ...
      <section className="mt-7 grid items-start gap-6 xl:grid-cols-[1.25fr_.75fr]">
        <div>
          ...
          <div className="divide-y divide-[#dfe5dc]">
            {alerts.map((alert) => {
              ...
              return <button key={alert.id} type="button" onClick={() => markAsRead(alert.id)} className={`grid w-full grid-cols-[5px_1fr_auto] gap-4 py-5 text-left hover:bg-[#fff7f9] ${alert.is_read ? 'opacity-60' : ''}`}>
                ...
                <span className="self-center text-right text-xs font-bold text-rose-800">{alert.is_read ? '확인됨' : urgent ? '센서 확인' : '내용 확인'}</span>
              </button>
            })}
          </div>
        </div>
        <aside className="border-l border-[#d9e0d7] pl-0 xl:pl-6">
          ...
          <dl className="mt-5 divide-y divide-[#dfe5dc] border-y border-[#dfe5dc]">
            <div className="flex items-baseline justify-between py-4"><dt className="text-sm text-slate-500">정상 센서</dt><dd className="text-2xl font-semibold tracking-[-.05em] tabular-nums text-emerald-800">2<span className="ml-1 text-sm font-medium">/ 4</span></dd></div>
            <div className="flex items-baseline justify-between py-4"><dt className="text-sm text-slate-500">체험 예약 대기</dt><dd className="text-2xl font-semibold tracking-[-.05em] tabular-nums text-[#223529]">1<span className="ml-1 text-sm font-medium">건</span></dd></div>
            <div className="flex items-baseline justify-between py-4"><dt className="text-sm text-slate-500">재고 주의 품목</dt><dd className="text-2xl font-semibold tracking-[-.05em] text-amber-700">1<span className="ml-1 text-sm font-medium">품목</span></dd></div>
          </dl>
        </aside>
      </section>

      <section className="mt-8">
        <article className="border-t border-[#b9c7b9] pt-4"><div><h2 className="section-title">꽃 재고 현황</h2><p className="section-description">판매 가능 수량 · 주 단위</p></div><div className="mt-4 h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={dashboardFlowerInventory} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}><CartesianGrid vertical={false} stroke="#dfe5dc" strokeDasharray="3 3" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#59665d', fontSize: 11 }} dy={8} /><YAxis axisLine={false} tickLine={false} tick={{ fill: '#748077', fontSize: 10 }} /><Tooltip contentStyle={{ borderRadius: 4, border: '1px solid #d5ddd4', boxShadow: 'none' }} formatter={(value) => [`${value}주`, '재고']} /><Bar dataKey="stock_quantity" radius={[2, 2, 0, 0]} maxBarSize={48}>{dashboardFlowerInventory.map((item, index) => <Cell key={item.name} fill={index === 3 ? '#bb7d26' : '#d15b86'} />)}</Bar></BarChart></ResponsiveContainer></div></article>
      </section>
    </div>
  )
}
